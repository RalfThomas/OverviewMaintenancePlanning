...
sap.ui.define([
	"sap/ui/base/Object",
	"zi2d/eam/maintenanceplanner/ovps1/util/StatusActionType",
	"zi2d/eam/maintenanceplanner/ovps1/util/StatusActionOptions"
], function(BaseObject, StatusActionType, StatusActionOptions) {
	"use strict";
	return BaseObject.extend("zi2d.eam.maintenanceplanner.ovps1.util.ActionHandlerProvider", {
		...
		onTechnicalCompleteAction: function(oView, oEvent) {
			var oActionModel = oView.getModel("ZEAM_ORDER_MONITOR_SRV"),
		...
		_raiseTecoDialog: function(oView, oInputModel) {
			this._oDialog = sap.ui.xmlfragment("zi2d.eam.maintenanceplanner.ovps1.view.DateSelectorDialog", this);
			...
			oView.getModel("ZEAM_ORDER_MONITOR_SRV").getSecurityToken();
		},
		...
		onAcceptStatusDialog: function(oEvent) {
			...
			var oModel = oEvent.getSource().getModel("ZEAM_ORDER_MONITOR_SRV");
		...
		_handleResultPromise: function(oPromise, oModel) {
			...
			oPromise.then(function(oResponse) {
				...
				if(oModel && oResponse && oResponse.data && oResponse.data.MaintenanceOrder) {
					var aMessages = oModel.getMessagesByEntity(oModel.createKey("ZC_MaintOrderForCompletionQ", { MaintenanceOrder : oResponse.data.MaintenanceOrder }));
